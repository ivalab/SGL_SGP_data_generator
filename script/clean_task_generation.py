    ...
    while cnt < args.number_data:
        ...
        while not scene_rendered:
            ...
            while attempt < args.max_attempt_per_scene and not scene_rendered:
                ...
                if task_type == 'inside':
                    spawn_obj_over_obj(controller, obj1_type, obj2_type,
                                       random.sample(small_objects, int(len(small_objects) * 0.9)) + containable_objects,
                                       False, 0.05)
                    scene_rendered = True
                elif task_type == 'outside':
                    scene_rendered = spawn_object_near_sink(controller, descriptions['scene']['kitchen'][scene_idx],
                                                 obj1_type, obj2_type, 'CounterTop', 0.05,
                                                 removed_objects +
                                                 random.sample(small_objects, int(len(small_objects) * 0.9)) + containable_objects, False)
        ...
        with open(os.path.join(args.save_root, str(cnt), 'pddl_goal_state.txt'), 'w+') as f:
            f.write('clean {} {}'.format(obj2_type.lower(), obj1_type.lower()))
