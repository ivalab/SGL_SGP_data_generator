...
from utils.agent_movement import teleport_face_2_obj, teleport_face_2_pos
from utils.objects import spawn_objects_wo_reps, spawn_objects_w_workingrange, isObjExist, isVisible
...
def main(args):
    ...
    obj_pick = descriptions['scene']['kitchen']['objects']['pick']
    obj_place = descriptions['scene']['kitchen']['objects']['place']
    ...
    while cnt < args.number_data:
        ...
        while not scene_rendered:
            ...
            while attempt < args.max_attempt_per_scene and not scene_rendered:
                obj1_type = random.choice(obj_pick)
                obj2_type = random.choice(obj_place)
                ...
                radius = descriptions['scene']['kitchen']['objects']['radius'][obj2_type]
                workrange = [[radius, radius+0.15], [[0, 360]]]
                scene_rendered = spawn_objects_w_workingrange(controller, workrange,
                                                              obj2_type, obj1_type,
                                                              removed_objects +
                                                              random.sample(small_objects, int(len(small_objects) * 0.7)),
                                                              yoffset=0.05, ifyoffset=True)

                # target_x, target_z = 0, 0
                # for obj in controller.last_event.metadata["objects"]:
                #     if obj["objectType"] == obj1_type or obj["objectType"] == obj2_type:
                #         target_x += obj['position']['x']
                #         target_z += obj['position']['z']
                #
                # target_x /= 2
                # target_z /= 2
                # teleport_face_2_pos(controller, target_x, target_z, ifLookDown=True)
        ...
        with open(os.path.join(args.save_root, str(cnt), 'transformations.json'), 'w+') as f:
            ...
            anno['pick_object'] = {}
            anno['pick_object']['position'] = obj1['position']
            anno['pick_object']['rotation'] = obj1['rotation']

            # cuttable object realted information
            anno['place_object'] = {}
            anno['place_object']['position'] = obj2['position']
            anno['place_object']['rotation'] = obj2['rotation']
        ...
        with open(os.path.join(args.save_root, str(cnt), 'pddl_goal_state.txt'), 'w+') as f:
            f.write('contain {} {}'.format(obj1_type.lower(), obj2_type.lower()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('save_root', type=str, default='../data/official/pick_n_place',
    ...
    parser.add_argument('--debug', action='store_true', help='whether to visualize the scene')
