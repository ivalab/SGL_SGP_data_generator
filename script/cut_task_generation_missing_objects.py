    ...
    while cnt < args.number_data:
            ...
            if miss_type == 0:
                ...
                while attempt < args.max_attempt_per_scene and not scene_rendered:
                    ...
                    if obj_observed == 1:
                        ...
                        # th knife and butterknife should be missing
                        scene_rendered &= not isVisible(controller, "Knife")
                        scene_rendered &= not isVisible(controller, "ButterKnife")
        ...
        with open(os.path.join(args.save_root, str(cnt), 'pddl_goal_state.txt'), 'w+') as f:
            if miss_type == 0 and obj_observed == 1:
                f.write('cut {} {}'.format("unknown", obj1_type.lower()))
            elif miss_type == 0 and obj_observed == 2:
                f.write('cut {} {}'.format(obj2_type.lower(), "unknown"))
            elif miss_type == 1:
                f.write('cut {} {}'.format("unknown", "unknown"))
            else:
                raise ValueError
